...
def _call_zhipu(api_key, messages, temperature=0.7):
    ...
    resp = client.chat.completions.create(model="glm-4-flash", messages=messages, temperature=temperature, timeout=60)
...
【Python代码加注释】如果内容包含Python代码，在每段代码后用#号注释说明该代码的含义和输出结果。
格式：# 这段代码的作用是...，输出为 [...]
